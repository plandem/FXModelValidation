...
```
- read this guide, [Validating Input](https://github.com/plandem/FXModelValidation/blob/master/Validation%20Input.md), [Core Validators](https://github.com/plandem/FXModelValidation/blob/master/Core%20Validators.md) and check out the [API documentation](http://cocoadocs.org/docsets/FXModelValidation)
...
_But let's start from begining..._
...
A model uses the **scenario** property to keep track of the scenario it is being used in. By default, a model supports only a single scenario named _default_. The following code shows how to set set the scenario of a model:
...
This library must be considered as independed project, with own codebase that was created from the ground.
